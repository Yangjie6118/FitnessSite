...
class RegForm(forms.Form):
    username = forms.CharField(label='用户名', max_length=30, min_length=5, widget=forms.TextInput(
                               attrs={'class': 'form-control', 'placeholder': '请输入用户名，5~30字符'}))
...
class NickNameForm(forms.Form):
    nickname = forms.CharField(
        label='新的昵称', max_length=20, widget=forms.TextInput(
...
class BindEmailForm(forms.Form):
    ...
    verification_code = forms.CharField(label='验证码', required=False, widget=forms.TextInput(
                                attrs={'class': 'form-control', 'placeholder': '点击“发送验证码”发送到邮箱'}))  # 验证码
...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again = forms.CharField(label='请再次输入新的密码', widget=forms.PasswordInput(
